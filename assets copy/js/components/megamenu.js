$(function () {
  ...
  $document.on('click', '.menu-item > a, .mega-item', function (ev) {
    ...
    if ($(window).innerWidth() < 1024) {
      ...
      if (hasChildren) {
        $this.siblings('.sub-menu').removeClass('bounceOutLeft').addClass('animated bounceInLeft active');
  ...
  $document.on('click', '.js-back', function (ev) {
    ...
    toHide.removeClass('bounceInLeft').addClass('animated bounceOutLeft');
    ...
    var $this = $(this)
      ...
      , closeMobNav = function (ev) {
          if ($(ev.target).closest('.megamenu-list').length === 0) {
            ...
            $controls.find(subMenus).removeClass('bounceInLeft bounceOutLeft');
          } else {
